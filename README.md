...
## Collections
**hashmaps->sparse-table**<br>`(hashmaps->sparse-table datum)`<br>`(hashmaps->sparse-table datum null-type)`<br>`(hashmaps->sparse-table datum null-type order-by)`<br><br>
...
**meta-iterate**<br>`(meta-iterate data-set)`<br><br>
...
**map-longest**<br>`(map-longest fn & colls)`<br><br>
...
**filter-map-key**<br>`(filter-map-key filter-fn some-map)`<br><br>
...
**filter-map-val**<br>`(filter-map-val filter-fn some-map)`<br><br>
...
**jaccard-words**<br>`(jaccard-words some-str some-another-str)`<br><br>
...
**pad-numbers**<br>`(pad-numbers n c pad-symbol)`<br><br>
...
**nested-group-by**<br>`(nested-group-by fs coll & [final-fn])`<br><br>
...
**pad-coll**<br>`(pad-coll n coll pad-with)`<br><br>
...
**order-by-collection**<br>`(order-by-collection input order-coll)`<br><br>
...
**invert-map**<br>`(invert-map dataset)`<br><br>
...
## Scales
**x-y->tile-lat-lon**<br>`(x-y->tile-lat-lon xtile ytile zoom)`<br><br>
...
**paginate**<br>`(paginate current last-item)`<br><br>
...
**lat-lon->tile-x-y**<br>`(lat-lon->tile-x-y lat-deg lon-deg zoom)`<br><br>
...
**log-scale**<br>`(log-scale x1 x2 y1 y2 x)`<br><br>
...
**scale**<br>`(scale A B C D X)`<br><br>
...
**log-scale-round**<br>`(log-scale-round x1 x2 y1 y2 x)`<br><br>
...
**golden-ratio**<br>`(golden-ratio size step)`<br><br>
...
**euclidean-distance**<br>`(euclidean-distance vec-a vec-b)`<br><br>
...
**font-size-in-a-tag-cloud**<br>`(font-size-in-a-tag-cloud min-font-size max-font-size items-in-the-biggest-tag items)`<br><br>
...
**tag-font-log-normalized**<br>`(tag-font-log-normalized items-in-the-biggest-tag items)`<br><br>
...
**haversine**<br>`(haversine lat-1 lng-1 lat-2 lng-2)`<br>`(haversine lat-1 lng-1 lat-2 lng-2 radius)`<br><br>
...
## Strings
**truncate-words-by-chars**<br>`(truncate-words-by-chars amount input)`<br>`(truncate-words-by-chars amount input ending)`<br><br>
...
**number->roman**<br>`(number->roman number-int)`<br><br>
Number to Roman<br>
**Usage:**
```clojure
...
**human-date**<br>`(human-date year month day)`<br><br>
...
**pluralize->as-ies**<br>`(pluralize->as-ies root-str number-int)`<br><br>
...
**number-ordinal->english**<br>`(number-ordinal->english number-int)`<br><br>
...
**lorem-ipsum**<br>`(lorem-ipsum amount-of-words)`<br>`(lorem-ipsum amount-of-words rand-fn)`<br><br>
...
**remove-new-lines**<br>`(remove-new-lines datum)`<br><br>
...
**pluralize->as-s**<br>`(pluralize->as-s root-str number-int)`<br><br>
...
**number->english**<br>`(number->english number-int)`<br><br>
...
## Framerate
**format-frames**<br>`(format-frames item)`<br><br>
null<br>
**Usage:**
```clojure
...
**number->frame-rate**<br>`(number->frame-rate frames frame-rate)`<br><br>
...
## HTML
**styles-map->string**<br>`(styles-map->string data)`<br><br>
...
**gen-layout-class**<br>`(gen-layout-class pivot a b c)`<br><br>
...
**catcher**<br>`(catcher err-binding on-error & body)`<br><br>
...
**html->unescaped-html**<br>`(html->unescaped-html some-str)`<br><br>
null<br>
**Usage:**
```clojure
...
**backlet**<br>`(backlet text class-item)`<br><br>
...
**grid-amount**<br>`(grid-amount size amount)`<br><br>
...
**parse-sorting-field**<br>`(parse-sorting-field input)`<br><br>
...
**inlet**<br>`(inlet text text-class)`<br><br>
...
## Transliterate
**cyrillic-str->latin**<br>`(cyrillic-str->latin phrase-str)`<br><br>
null<br>
**Usage:**
```clojure
...
**latin-str->cyrillic**<br>`(latin-str->cyrillic phrase-str)`<br><br>
null<br>
**Usage:**
```clojure
...
## FS
**temp-file**<br>`(temp-file)`<br>`(temp-file prefix extension)`<br><br>
null<br>
**Usage:**
```clojure
...
**zip-dir**<br>`(zip-dir archive-name directory)`<br><br>
