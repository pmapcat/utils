...
## Collections
**hashmaps->sparse-table**<br>`(hashmaps->sparse-table datum)`
...
**meta-iterate**<br>`(meta-iterate data-set)`<br>
...
**map-longest**<br>`(map-longest fn & colls)`<br>
...
**filter-map-key**<br>`(filter-map-key filter-fn some-map)`<br>
...
**filter-map-val**<br>`(filter-map-val filter-fn some-map)`<br>
...
**jaccard-words**<br>`(jaccard-words some-str some-another-str)`<br>
...
**pad-numbers**<br>`(pad-numbers n c pad-symbol)`<br>
...
**nested-group-by**<br>`(nested-group-by fs coll & [final-fn])`<br>
...
**pad-coll**<br>`(pad-coll n coll pad-with)`<br>
...
**order-by-collection**<br>`(order-by-collection input order-coll)`<br>
...
**invert-map**<br>`(invert-map dataset)`<br>
...
## Scales
**x-y->tile-lat-lon**<br>`(x-y->tile-lat-lon xtile ytile zoom)`<br>
...
**paginate**<br>`(paginate current last-item)`<br>
...
**lat-lon->tile-x-y**<br>`(lat-lon->tile-x-y lat-deg lon-deg zoom)`<br>
...
**log-scale**<br>`(log-scale x1 x2 y1 y2 x)`<br>
...
**log-scale-round**<br>`(log-scale-round x1 x2 y1 y2 x)`<br>
...
**golden-ratio**<br>`(golden-ratio size step)`<br>
...
**euclidean-distance**<br>`(euclidean-distance vec-a vec-b)`<br>
...
**font-size-in-a-tag-cloud**<br>`(font-size-in-a-tag-cloud min-font-size max-font-size items-in-the-biggest-tag items)`<br>
...
**tag-font-log-normalized**<br>`(tag-font-log-normalized items-in-the-biggest-tag items)`<br>
...
**haversine**<br>`(haversine lat-1 lng-1 lat-2 lng-2)`
...
## Strings
**truncate-words-by-chars**<br>`(truncate-words-by-chars amount input)`
...
**number->roman**<br>`(number->roman number-int)`<br>
Number to Roman<br>
**Usage:**
```clojure
...
**human-date**<br>`(human-date year month day)`<br>
...
**pluralize->as-ies**<br>`(pluralize->as-ies root-str number-int)`<br>
...
**number-ordinal->english**<br>`(number-ordinal->english number-int)`<br>
...
**lorem-ipsum**<br>`(lorem-ipsum amount-of-words)`
...
**remove-new-lines**<br>`(remove-new-lines datum)`<br>
...
**pluralize->as-s**<br>`(pluralize->as-s root-str number-int)`<br>
...
**number->english**<br>`(number->english number-int)`<br>
...
## Framerate
**format-frames**<br>`(format-frames item)`<br>
null<br>
**Usage:**
```clojure
...
**number->frame-rate**<br>`(number->frame-rate frames frame-rate)`<br>
...
## HTML
**styles-map->string**<br>`(styles-map->string data)`<br>
...
**gen-layout-class**<br>`(gen-layout-class pivot a b c)`<br>
...
**catcher**<br>`(catcher err-binding on-error & body)`<br>
...
**html->unescaped-html**<br>`(html->unescaped-html some-str)`<br>
null<br>
**Usage:**
```clojure
...
**backlet**<br>`(backlet text class-item)`<br>
...
**grid-amount**<br>`(grid-amount size amount)`<br>
...
**parse-sorting-field**<br>`(parse-sorting-field input)`<br>
...
**inlet**<br>`(inlet text text-class)`<br>
...
## Transliterate
**cyrillic-str->latin**<br>`(cyrillic-str->latin phrase-str)`<br>
null<br>
**Usage:**
```clojure
...
**latin-str->cyrillic**<br>`(latin-str->cyrillic phrase-str)`<br>
null<br>
**Usage:**
```clojure
...
**zip-dir**<br>`(zip-dir archive-name directory)`<br>
